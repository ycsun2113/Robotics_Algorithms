...
def linearize_dynamics_numerically(x_r, u_r, h, true_dynamics):
    ...
    for col in range(n):
        xu_r_h = np.copy(xu_r)
        xu_r_h[col] += h
        Jacobian[:, col] = (f(xu_r_h) - f(xu_r))/h
...
def optimize_single_action(goal_state, current_state, reference_control, A, B, speed_limit, turn_limit):
    ...
    ### YOUR CODE HERE ###
    # control[0]: speed, control[1]: turn
    # speed_limit[0]: min_speed, speed_limit[1]: max_speed
    # turn_limit[0]: min_turn, turn_limit[1]: max_turn
    P = B.T @ B
    q = - (goal_state - A @ current_state).T @ B
    r = (1/2) * (goal_state - A @ current_state).T @ (goal_state - A @ current_state)
