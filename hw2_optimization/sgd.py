...
def sgd(step_size=1, max_iters=1000, x_0=-5):
    ...
    for k in range(max_iters):
        ...
        # delta_x(k) = - grad_fi(x(k))
        delta_x = -1 * SGDtest.fiprime(x_next, i)

        # x(k+1) = x(k) + t(k) * delta_x(k)
        x_next += step_size * delta_x
...
def run_sgd(num=30, iters=1000):
    ...
    for i in range(num):
        xi, x_opt = sgd(step_size=1, max_iters=iters, x_0=-5)
...
# SGD
sgd_start = time.time()
sgd_xi, sgd_xopt = sgd(step_size=1, max_iters=1000, x_0=x_0)
sgd_end = time.time()
