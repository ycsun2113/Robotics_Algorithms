...
def main():
    ...
    for i in range(max_iter):
        ...
        # Compute null space:
        J_pinv_J = J_pinv @ J
        I = np.identity(J_pinv_J.shape[0])
        N = (I - J_pinv_J)

        # Find q_dot_second:
        lambda_for_q_dot_second = 0.5
        q_dot_second = np.array([0, 0, 0, 0, 0, 0, 0])
        for i in range(len(joint_idx)):
            q_min = joint_limits[joint_names[i]][0]
            q_max = joint_limits[joint_names[i]][1]
            if(abs(q_min - q_current[i]) <= abs(q_max - q_current[i])):
                q_dot_second[i] = lambda_for_q_dot_second
            else:
                q_dot_second[i] = -lambda_for_q_dot_second

        # q_dot with null-space:
        q_dot = (J_pinv @ x_dot) + (beta * (N @ q_dot_second))
